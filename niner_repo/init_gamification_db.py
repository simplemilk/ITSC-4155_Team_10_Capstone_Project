...
def init_gamification_db():
    ...
    # First, drop all existing gamification objects
    print("🗑️  Dropping existing gamification objects...")
    try:
        cursor.execute('DROP INDEX IF EXISTS idx_user_progress_user')
        cursor.execute('DROP INDEX IF EXISTS idx_achievements_user')
        cursor.execute('DROP INDEX IF EXISTS idx_achievements_milestone')
        cursor.execute('DROP INDEX IF EXISTS idx_activities_user')
        cursor.execute('DROP INDEX IF EXISTS idx_activities_date')
        ...
        cursor.execute('DROP TABLE IF EXISTS game_activities')
        ...
        cursor.execute('DROP TABLE IF EXISTS user_game_progress')
        conn.commit()
    ...
    tables = cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND (name LIKE '%game%' OR name LIKE '%milestone%' OR name LIKE '%badge%' OR name LIKE '%level%' OR name LIKE '%streak%' OR name LIKE '%achievement%')"
    ).fetchall()
    
    conn.close()
    
    expected_tables = ['user_game_progress', 'milestones', 'user_achievements', 'levels', 
                      'game_activities', 'user_streaks', 'badges', 'user_badges']
    ...
    if len(tables) >= len(expected_tables):
        ...
        print(f"❌ Failed to create all gamification tables")
        print(f"  Created: {', '.join([t[0] for t in tables])}")
        print(f"  Expected: {', '.join(expected_tables)}")
        return False

def create_tables_inline(cursor, conn):
    """Create gamification tables inline (fallback method)"""
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_user_progress_user ON user_game_progress(user_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_achievements_user ON user_achievements(user_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_achievements_milestone ON user_achievements(milestone_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_activities_user ON game_activities(user_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_activities_date ON game_activities(created_at)')
